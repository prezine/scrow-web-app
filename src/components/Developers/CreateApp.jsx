...
const CreateApp = ({closeModal, setOpenAlert, setAlertValues, alertValues, mutate}) => {
  ...
  const handleSubmit = (e) => {
    ...
    formData.append('appname', formik.values.appname)
    formData.append('appdescription', formik.values.appdescription)
    formData.append('display_name', formik.values.display_name)
    formData.append('industry', formik.values.industry)
    formData.append('products', formik.values.products)
    formData.append('scope', formik.values.scope)
    formData.append('appicon', uploadedImageUrl ? uploadedImageUrl : '')

    try {

      axios.post(`${import.meta.env.VITE_BASEURL}/app/create?userID=${userDataValue && userDataValue.userID}`, formData, requestHeaders)
      .then((res)=>{
        console.log(res);
          ...
          }else if (res.data.status == true && res.data.data.appid) {
              formik.resetForm()
              setOpenAlert(true)
              setAlertValues({...alertValues, message:`${res.data.data.appname} created successfully!`, type:`success` })
              mutate()
              closeModal()
          }
  ...
  return (
    <div className='bg-white relative m-auto rounded-ten py-8 px-5 md:py-8 md:px-8 lg:px-14 z-50 w-ninetyFivePercent sm:w-sixtyFivePercent md:w-sixtyPercent lg:w-fiftyPercent h-fit'>
      <div className='text-center'>
        <h4 className='text-2xl md:text-3xl pb-1 text-brandGray14x font-avenirHeavy'>Create an App</h4>
      </div>
      ...
      <FormFileInput formik={formik} fileValue={formik.values.appicon} errorUpload={errorUpload} setErrorUpload={setErrorUpload} setUploadedImageUrl={setUploadedImageUrl} handleCancel={handleCancel} handleFileUpload={handleFileUpload} uploading={uploading} uploadedImageUrl={uploadedImageUrl} actionText={'Choose App Icon'} fileInputId={'appicon'} fileInputName={'appicon'} handleBlur={formik.handleBlur} fieldError={formik.touched.appicon && formik.errors.appicon} />
        ...
        <div>
          <FormTextArea maxLength={500} textAreaId={'appdescription'} textAreaName={'appdescription'} textAreaPlaceholder={"Describe what the product you're building look like"} textAreaLabel={'Explain what this App would do'} handleChange={formik.handleChange} handleBlur={formik.handleBlur} inputValue={formik.values.appdescription} fieldError={formik.touched.appdescription && formik.errors.appdescription} />
        </div>

        <div className="col-span-1 mds:col-span-2 sm:col-span-2 pt-5 flex items-center justify-end">
          <ButtonPrimary handleClick={handleSubmit} disabled={submitting} disabledBgColor={'bg-brandGray16x'} text={'Create New App'} type={'button'} />
        </div>
      </form>
    </div>
  )
