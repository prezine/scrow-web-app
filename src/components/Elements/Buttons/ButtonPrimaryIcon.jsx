...
const ButtonPrimaryIcon = ({id, fontSize, handleClick, type, icon, svgStrokeColor, fontStyle, flexDirection, otherTextStyles, disabled, disabledBgColor, gap, textColor, text, bgColor, paddingY, paddingX, borderRadius, width, textWrap}) => {
  return (
    <button type={type || 'button'} onClick={handleClick} id={id} disabled={disabled} className={`${flexDirection} flex items-center ${gap || 'gap-2.5'} h-fit ${width ? width : 'w-fit'} ${paddingY || 'py-1.5'} ${textWrap ? 'whitespace-nowrap' : 'whitespace-normal'} ${paddingX || 'px-3.5'} ${borderRadius || 'rounded-five'} ${disabled ? `${disabledBgColor}` : ''} ${textColor || 'text-white'} ${bgColor || 'bg-brandDarkViolet1x'} ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
        {
          icon
          ?
          icon
        ...
        <span className={`${fontSize || 'md:text-md '} ${otherTextStyles} ${fontStyle || 'font-spaceGroteskRegular'}`} >{text || 'New Transaction'}</span>
    </button>
  )
}
