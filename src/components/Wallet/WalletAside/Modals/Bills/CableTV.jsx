...
const CableTV = ({currency, currentButtonText, openAlert, alertValues, setOpenAlert, setAlertValues, closeModal, mutate, balance}) => {
  ...
  const [variationAmount, setVariationAmount] = useState(0.00)
  ...
  const utilityListUrl = `${import.meta.env.VITE_BASEURL}/bill/lists?category=cable&country=NG&userID=${userDataValue && userDataValue.userID}`;
  ...
  const handleVarAmount = (e) => {
    formik.setFieldValue('variation', e.target.value)
    const var_amount = e.target.options[e.target.selectedIndex].getAttribute('data-variation-amount');
    setVariationAmount(var_amount || 0.00)

    formik.setFieldValue('amount', var_amount.split('.')[0].replaceAll(','))
  }
  ...
  const handleSubmit = () => {
    ...
    formData.append('variation', formik.values.variation.split('@')[0].trim())
  ...
  return (
    <form action="" method="post"  className={`flex flex-col gap-5 pt-5`}>
      <fieldset className={`gap-2.5 flex flex-col col-span-1 mds:col-span-2 sm:col-span-2`}>
        <label htmlFor="serviceID" className='text-xs font-spaceGroteskMedium text-brandGray14x text-left'>{`Select your Cable Provider`}</label>
        <div className="relative w-full">
            <select type="text" disabled={utilityList ? !utilityList.data || utilityList.error : true} required name='serviceID' id='serviceID' onChange={formik.handleChange} onBlur={formik.handleBlur} value={formik.values.serviceID} className={`px-4 py-2.5 w-full h-11 bg-transparent font-spaceGroteskRegular text-sm text-black placeholder:text-brandGray32x rounded-five border-2 ${(formik.touched.serviceID && formik.errors.serviceID) || (submitting && formik.errors.serviceID) ? 'border-brandRed1x focus:border-brandRed1x' : 'border-brandGray16x focus:border-black'} focus:border-2 focus:outline-none w-full`} >
                <option value="" selected disabled>Select your cable provider</option>
      ...
      <fieldset className={`gap-2.5 flex flex-col col-span-1 mds:col-span-2 sm:col-span-2`}>
        <label htmlFor="variation" className='text-xs font-spaceGroteskMedium text-brandGray14x text-left'>{`Select your Package`}</label>
        <div className="relative w-full">
            <select type="text" disabled={utilityList ? !utilityList.data && !variationsList.data || utilityList.error && variationsList.error : true} required name='variation' id='variation' value={formik.values.variation} onChange={(e) => {handleVarAmount(e)}} onBlur={formik.handleBlur} className={`px-4 py-2.5 w-full h-11 bg-transparent font-spaceGroteskRegular text-sm text-black placeholder:text-brandGray32x rounded-five border-2 ${(formik.touched.variation && formik.errors.variation) || (submitting && formik.errors.variation) ? 'border-brandRed1x focus:border-brandRed1x' : 'border-brandGray16x focus:border-black'} focus:border-2 focus:outline-none w-full`} >
                ...
                {variationsList && variationsList.data && variationsList.data.data && variationsList.data.status && variationsList.data.data.variations.length > 0 && variationsList.data.data.variations.map((variation, idx)=>{
                  return <option key={idx} data-variation-amount={variation.variation_amount} value={`${variation.variation_code} @ ${variation.name.toLowerCase()}`}>{variation.name}</option>
                })}
            </select>
        </div>
      </fieldset>
      <div>
          <fieldset className={`gap-2.5 flex flex-col col-span-1 mds:col-span-2 sm:col-span-2`}>
              <label htmlFor="amount" className='text-xs font-spaceGroteskMedium text-brandGray14x text-left'>{`Preview Amount of Package`}</label>
              <div className="relative w-full">
                  <input required disabled name='amount' id='amount' value={Intl.NumberFormat('en', {minimumFractionDigits:2}).format(variationAmount)} onChange={(e) => {useAmountFormat(e, formik, 'amount')}} onBlur={formik.handleBlur} placeholder='Amount' className={`pl-12 pr-4 py-2.5 w-full font-spaceGroteskRegular disabled:bg-brandGray49x disabled:border-0.5 disabled:border-brandGray17x text-sm text-black placeholder:text-brandGray32x rounded-five border-2 ${(formik.touched.amount && formik.errors.amount) || (submitting && formik.errors.amount) ? 'border-brandRed1x focus:border-brandRed1x' : 'border-brandGray16x focus:border-black'} focus:border-2 focus:outline-none w-full`} />
